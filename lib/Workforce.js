...
class Workforce {
    ...
    getNextId() {
        const numIds = this.workforceData.length;

        if (!numIds) return 1;

        for (let i = 0; i < numIds - 1; i++) {
            // Use every possible integer
            const nextOrdinal = i + 1;
            if (this.workforceData[i] > nextOrdinal) {
                return nextOrdinal;
            }
        }
        // All possible numbers used - increment current highest
        return this.workforceData.pop() + 1;
    }
}
